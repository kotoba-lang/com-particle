(ns particle.main
  "Kotodama WASM entrypoint for the Particle clean-room actor (L4) — Clojure port.
  ...
  facts (`particle.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "particle")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"particle.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "particle-compat" :tier tier :entities entities} 200])
...
(defn start! [] :particle-compat/ready)
